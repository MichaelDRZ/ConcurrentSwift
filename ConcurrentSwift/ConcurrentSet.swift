...
public extension Set {
    ...
    /// - Returns: A set containing only the elements for which isIncluded returns true.
    
    func concurrentFilter(_ isIncluded: (Element) -> Bool) -> Set<Element> {
        guard !isEmpty else { return Set<Element>() }
        ...
        return Set(batchResults.flatMap({ $0! }))
    }
}
